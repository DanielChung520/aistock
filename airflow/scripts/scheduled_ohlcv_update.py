...
def _fetch_twse_daily_month(stock_id: str, year: int, month: int, max_retries: int = 3) -> list[dict[str, Any]]:
    """Fetch daily K-line data for one month from TWSE."""
    import importlib

    twse_stock = importlib.import_module("twstock.stock")

    for attempt in range(max_retries):
        try:
            twse_rate_limiter.wait()
            fetcher = twse_stock.TWSEFetcher()
...
def main() -> None:
    ...
    if args.stock_id:
        ...
        stocks = _get_watchlist(db)

    if not stocks:
        logger.info("No stocks to update (watchlist is empty)")
        return
